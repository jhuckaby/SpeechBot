...
module.exports = Class.create({
	...
	firehose: function(cmd, value, chat) {
		...
		if (cmd in this.data.faqs) {
			...
			if (chat.username == this.bot.config.getPath('connection.username')) {
				if (this.lastSelfReply && (Tools.timeNow() - this.lastSelfReply < 1.0)) return;
				this.lastSelfReply = Tools.timeNow();
			}
