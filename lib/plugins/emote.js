...
module.exports = Class.create({
	...
	cmd_clever: function(value, chat) {
		// add/delete/list clever triggers
		if (value.match(/^(add|set)\s+(.+)$/i)) {
			// add word
			var word = RegExp.$2.toLowerCase();
			if (word in this.data.words) {
				return this.doError(chat, "Word already in bot trigger list: " + word);
			}
			this.data.words[word] = 1;
			this.compile();
			this.storeData();
			this.doReply(chat, ":robot_face: Word added to bot trigger list: " + word);
		}
		else if (value.match(/^(delete|remove)\s+(.+)$/i)) {
			// delete word
			var word = RegExp.$2.toLowerCase();
			if (!(word in this.data.words)) {
				return this.doError(chat, "Word not found in bot trigger list: " + word);
			}
			delete this.data.words[word];
			this.compile();
			this.storeData();
			this.doReply(chat, ":robot_face: Word removed from bot trigger list: " + word);
		}
		else if (value.match(/^list/i)) {
			// list all words
			var words = Object.keys(this.data.words).sort();
			if (!words.length) this.doReply(chat, "There are currently no words in the bot trigger list.");
			else this.doReply(chat, ":robot_face: The following words will trigger the bot: " + words.join(', '));
		}
		else {
			this.doError(chat, "Invalid !clever command syntax.");
		}
	},
	...
	clever: function(chat) {
		...
		else if (chat.text.match(this.regexp)) {
			// custom trigger word
